...
function Lobby:draw()
    ...
    local text = self.matchmake_state == 'searching' and 'Cancel' or
                     'Search Match'
    ...
    FriendsSidebar.draw(Suit, Fonts, Lobby)
    ...
    if play_button.hit then
        coroutine.resume(coroutine.create(function()
            ...
            if self.matchmake_state == 'searching' then
                ...
                socket.matchmaker_remove(Constants.SOCKET_CONNECTION,
                                         self.matchmake_ticket)
            else
                self.matchmake_state = 'searching'
                socket.matchmaker_add(Constants.SOCKET_CONNECTION, 2, 2, nil,
                                      nil, nil, nil, function(matchmake)
