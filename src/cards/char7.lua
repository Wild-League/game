...
function Char7:perception_range()
...
Char7.animate.update = function(self,dt)
	return self.actions[self.current_action].update(dt)
end

Char7.animate.draw = function(self,x, y, ...)
	self.lifebar(x,y)
	self.show_name(x,y)
...
Char7.actions = {
	...
	follow = {
		update = function(dt)
			nearest_enemy = Char7.get_nearest_enemy(Char7.chars_around)
	...
	attack = {
		...
		draw = function(x,y)
			...
			if (nearest_enemy.y > shoot.y) then
				shoot.y = shoot.y + Char7.attack_speed
			end
			if (nearest_enemy.y < shoot.y) then
				shoot.y = shoot.y - Char7.attack_speed
			end
			if (nearest_enemy.x > shoot.x) then
				shoot.x = shoot.x + Char7.attack_speed
			end
			if (nearest_enemy.x < shoot.x) then
				shoot.x = shoot.x - Char7.attack_speed
			end
        ...
        shoot.x = Char7.char_x
        shoot.y = Char7.char_y
    	end
...
function Char7.get_nearest_enemy(around)
	shoot.x = Char7.char_x
	shoot.y = Char7.char_y

	for k,v in pairs(around) do
		local distance_x = v.x - Char7.char_x
		local distance_y = v.y - Char7.char_y

		if (distance_x >= (nearest_enemy.x - Char7.char_x))
			and (distance_y >= (nearest_enemy.y - Char7.char_y)) then
			return v
		end
	end
end

function Char7.lifebar(x, y)
...
function Char7.show_name(x, y)
	love.graphics.print(Char7.name, x, y)
end
