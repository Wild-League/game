...
function Char8:perception_range()
...
Char8.animate.update = function(self,dt)
	return self.actions[self.current_action].update(dt)
end

Char8.animate.draw = function(self,x, y, ...)
	self.lifebar(x,y)
	self.show_name(x,y)
...
Char8.actions = {
	...
	follow = {
		update = function(dt)
			nearest_enemy = Char8.get_nearest_enemy(Char8.chars_around)
	...
	attack = {
		...
		draw = function(x,y)
			...
			if (nearest_enemy.y > shoot.y) then
				shoot.y = shoot.y + Char8.attack_speed
			end
			if (nearest_enemy.y < shoot.y) then
				shoot.y = shoot.y - Char8.attack_speed
			end
			if (nearest_enemy.x > shoot.x) then
				shoot.x = shoot.x + Char8.attack_speed
			end
			if (nearest_enemy.x < shoot.x) then
				shoot.x = shoot.x - Char8.attack_speed
			end
        ...
        shoot.x = Char8.char_x
        shoot.y = Char8.char_y
    	end
...
function Char8.get_nearest_enemy(around)
	shoot.x = Char8.char_x
	shoot.y = Char8.char_y

	for k,v in pairs(around) do
		local distance_x = v.x - Char8.char_x
		local distance_y = v.y - Char8.char_y

		if (distance_x >= (nearest_enemy.x - Char8.char_x))
			and (distance_y >= (nearest_enemy.y - Char8.char_y)) then
			return v
		end
	end
end

function Char8.lifebar(x, y)
...
function Char8.show_name(x, y)
	love.graphics.print(Char8.name, x, y)
end
