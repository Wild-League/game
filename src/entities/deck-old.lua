...
local DeckMetatable = {
	__call = function(self)
		...
		Deck.deck_selected = Constants.DECK_SELECTED

		if #self[Deck.deck_selected] > self.selectable_cards then
			-- if more than 4 cards, should rotate cards

			-- get the cards left from deck and make unselectable
			-- and add to queue
			for i = self.selectable_cards + 1, #self[Deck.deck_selected] do
				self[Deck.deck_selected][i].selectable = false
				table.insert(self.queue_next_cards, self[Deck.deck_selected][i])
			end
...
function Deck:define_positions()
	...
	local position = Layout:down_right(card1.card_img:getWidth(), card1.card_img:getHeight() - 25)
